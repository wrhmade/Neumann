...
file_t file_table[MAX_FILE_NUM];
...
int install_to_global(fileinfo_t finfo)
{
    int i = MAX_FILE_NUM;
    for (i = 0; i < MAX_FILE_NUM; i++) {
    ...
    fileinfo_t *safer_finfo = (fileinfo_t *) malloc(sizeof(fileinfo_t)); // 分配一个finfo指针，准备挂到handle上
