...
void init_ide(void)
{
	...
	/* 检测计算机是否拥有IDE控制器 */
	if (!pci_find_device_by_class(0x010100)) {//0x010100表示IDE控制器
		//print_warn("The IDE controller could not be found!\n");
        krnlcons_putstr_color("IDE controller not found.\n",0xFFFFFF,0x000000);
		return;
	}
	...
	if (ide_devices[0].Reserved == 0 && ide_devices[1].Reserved == 0) {
		if (ide_devices[2].Reserved == 0 && ide_devices[3].Reserved == 0) {
            ...
            krnlcons_putstr_color("Main IDE device not found.\n",0xFFFFFF,0x000000);
		}
	}
}
...
static void ide_initialize(uint32_t BAR0, uint32_t BAR1, uint32_t BAR2, uint32_t BAR3, uint32_t BAR4)
{
	...
	for (int i = 0; i < 2; i++) {
		for (j = 0; j < 2; j++) {
			...
			int device_io_in32ex = i * 2 + j;  // 计算设备索引
			ide_devices[device_io_in32ex].Reserved = 0;
			...
			ide_devices[device_io_in32ex].Reserved = 1;
			ide_devices[device_io_in32ex].Type = type;
			ide_devices[device_io_in32ex].Channel = i;
			ide_devices[device_io_in32ex].Drive = j;
			ide_devices[device_io_in32ex].Signature = *((uint16_t *)(ide_buf + ATA_IDENT_DEVICETYPE));
			ide_devices[device_io_in32ex].Capabilities = *((uint16_t *)(ide_buf + ATA_IDENT_CAPABILITIES));
			ide_devices[device_io_in32ex].CommandSets = *((uint32_t *)(ide_buf + ATA_IDENT_COMMANDSETS));

			if (ide_devices[device_io_in32ex].CommandSets & (1 << 26))
				ide_devices[device_io_in32ex].Size = *((uint32_t *)(ide_buf + ATA_IDENT_MAX_LBA_EXT));
			else
				ide_devices[device_io_in32ex].Size = *((uint32_t *)(ide_buf + ATA_IDENT_MAX_LBA));
			for (k = 0; k < 40; k += 2) {
				ide_devices[device_io_in32ex].Model[k] = ide_buf[ATA_IDENT_MODEL + k + 1];
				ide_devices[device_io_in32ex].Model[k + 1] = ide_buf[ATA_IDENT_MODEL + k];
			}
			ide_devices[device_io_in32ex].Model[40] = 0;
			detected[device_io_in32ex] = 1;  // 标记设备已检测
			count++;
		}
	}
	...
	for (int i = 0; i < 4; i++) {
		if (ide_devices[i].Reserved == 1 && detected[i]) {
			if (!info) {
				krnlcons_putstr("Found IDE Driver: ");
				if (!info) {
					sprintf(s,"Type: %s | %d(MiB) - %s\n", ide_devices[i].Type ? "ATAPI" :
...
void get_ide_info(int secondary,int slave,ide_info_t *info)
{
	int index=((secondary << 1) | slave);
	if(ide_devices[index].Reserved)
	{
		info->reserved=1;
