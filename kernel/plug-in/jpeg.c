 ...
 * 2003/09/27 | PICTURE0.BIN(DLL)�p�ɉ��� ( by ���[�݂� )
 * 2003/09/28 | �e��o�O�t�B�N�X�������̍œK�� ( by H.Kawai )
 *
 */
...
typedef struct
{
	...
	int base_img[64][64]; // ���摜 ( [�����g��u��][�c���g��v��][���ʑ�(M/8)][�c�ʑ�(N/8)]
...
unsigned short get_bits(JPEG *jpeg, int bit)
{
	...
	while (remain <= 16) {
		...
		c = *jpeg->fp++;
		if (c == 0xff) { // �}�[�J�G���[��h�����߁AFF -> FF 00 �ɃG�X�P�[�v����Ă�
			if (jpeg->fp >= jpeg->fp1) {
				ret = 0;
				goto fin;
			}
	...
	while (size > 0) {
		...
		if (j > jpeg->n_dqt)
 		...
 		if (c & 0xf8) {
			// 16 bit DQT
			for (i = 0; i < 64; i++) {
				jpeg->dqt[j][i] = jpeg->fp[0]; /* ���ʂ͓ǂݎ̂ĂĂ��� */
				jpeg->fp += 2;
			}
			size += -64 * 2;
		} else {
...
int jpeg_dht(JPEG *jpeg)
{
	...
	while (len > 0) {
		...
		tc = (val >> 4) & 0x0f; // �e�[�u���N���X(DC/AC�����Z���N�^)
		th =  val       & 0x0f; // �e�[�u���w�b�_(�����ڂ̃v���[����)
...
int jpeg_decode_mcu(JPEG *jpeg, UCHAR *zigzag_table)
{
	...
	// mcu_width x mcu_height�T�C�Y�̃u���b�N��W�J
